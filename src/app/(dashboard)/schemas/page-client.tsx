...
export default function SchemasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (matiere !== 'all') list = list.filter((s) => s.subject === SUBJECT_LABELS[matiere as keyof typeof SUBJECT_LABELS])
    ...
    <div className="max-w-350">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="mono text-xs px-2 py-1 rounded-full font-medium tabular-nums"
              ...
              {loading ? '…' : `${schemas.length} ${schemas.length === 1 ? 'schéma' : 'schémas'}`}
            </span>
          </div>
        </div>
      ...
      {!loading && schemas.length === 0 ? (
        <EmptyState Icon={GitBranch} color={COLOR} title={"Aucun schéma"}
          ...
          ctaLabel={"Nouveau schéma"} ctaHref="/schemas/new" />
      ) : (
