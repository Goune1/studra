...
export default function PlanningNewPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      toast.success(`Planning créé (${json.taskCount} ${json.taskCount === 1 ? "session" : "sessions"})`)
      ...
      {selected.length > 0 && (
        ...
        >
          <span className="text-sm" style={{ color: 'var(--ink-700)' }}>
            <strong style={{ color: 'var(--ink)' }}>{`${selected.length} ${selected.length === 1 ? "contenu" : "contenus"} sélectionné${selected.length === 1 ? "" : "s"}`}</strong>
          </span>
        </div>
      )}
