          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: 'var(--ink-500)' }}>
                {"Identifiant Pronote"}: {connection.username}
              </p>
            </div>
          </div>
          {lastSyncedAt && (
            <p className="mono text-xs" style={{ color: 'var(--ink-400)' }}>
              {`Dernière synchronisation : ${format.dateTime(new Date(lastSyncedAt), {day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit'})}`}
            </p>
          )}
          {!lastSyncedAt && (
