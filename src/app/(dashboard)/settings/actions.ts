...
export async function deleteAccount(confirmationEmail: string) {
  ...
  if (confirmationEmail.trim().toLowerCase() !== (user.email ?? '').toLowerCase()) {
    return { ok: false, error: "L'email saisi ne correspond pas à ton compte." }
  }
  ...
  if (deleteError) {
    ...
    if (anonymizeError) {
      return { ok: false, error: "Impossible de supprimer le compte. Réessaie ou contacte le support." }
    }
