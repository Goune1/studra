...
function ranking(results: SectionResult[], ascending: boolean) {
  return [...results].sort((a, b) => {
    const aRate = a.totalQuestions ? a.correctCount / a.totalQuestions : 0
    const bRate = b.totalQuestions ? b.correctCount / b.totalQuestions : 0
...
export default async function TageMageResultsPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const strengths = ranking(sectionResults, false)
  const priorities = ranking(sectionResults, true)
  ...
  return (
    <main className={styles.resultPage}>
      ...
      <section className={styles.resultHero}>
        <div><p className={styles.kicker}>Diagnostic terminé</p><h1>Ton résultat brut.</h1><p className={styles.resultCount}>{attempt.correct_count}/{attempt.total_questions}</p><p className={styles.resultLabel}>réponses correctes · {duration(attempt.duration_seconds)}</p></div>
        <p className={styles.notice}>Diagnostic Studra non officiel : ce résultat ne constitue pas une estimation de score TAGE MAGE ni un score sur 600.</p>
      </section>
      ...
      <section className={styles.resultSection} aria-labelledby="priorities"><h2 id="priorities">Pour la suite</h2><div className={styles.priorityGrid}><div><h3>Points d’appui</h3><ul className={styles.priorityList}>{strengths.map((result) => <li key={result.section}>{sectionLabel(result.section)} · {result.correctCount}/{result.totalQuestions}</li>)}</ul></div><div><h3>Priorités</h3><ul className={styles.priorityList}>{priorities.map((result) => <li key={result.section}>{sectionLabel(result.section)} · {result.correctCount}/{result.totalQuestions}</li>)}</ul></div></div></section>
