...
function QuestionCard({
  ...
  const typeColors: Record<string, { bg: string; fg: string; label: string }> = {
    ...
    open:     { bg: '#10B98115', fg: '#10B981', label: "Réponse" },
      ...
      <div className="flex items-start justify-between mb-6 animate-fade-up">
        <div>
          ...
          <p className="mono text-xs mt-2" style={{ color: 'var(--ink-500)' }}>
            {`${exam.questions_json.length} ${exam.questions_json.length === 1 ? 'question' : 'questions'}`}
          </p>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          >
            {showAllAnswers ? "Réponse" : "Voir le corrigé"}
          </button>
          <button
